        ...
        IF(THIS%AEROF%MKFM .EQ. 6) THEN
            !IF(N .EQ. 1) THEN
                ...
                DO I=1,THIS%AEROF%NSTP
                    DO J=1,NPTS
                        READ(444,*) DROP,DROP,(THIS%AEROF%CFDFPSI(K,J,I),K=1,3),&
								                (THIS%AEROF%CFDMPSI(K,J,I),K=1,3)
                !    write(*,*) (THIS%AEROF%CFDFPSI(K,J,I),K=1,3),&
								    !(THIS%AEROF%CFDMPSI(K,J,I),K=1,3)
...
!->++++++++++++++++++++++++++++++++++++++++++++
        ...
        CALL THIS%OUTWVF()
        CALL THIS%OUTVMU()
        CALL THIS%GET_DEFORM(N,INPU%IPT%NumofRotors)
    !END DO
...
REAL(RDT),EXTERNAL :: FWM
    ...
    !输出共轴旋翼上下桨叶相遇方位处的桨叶展向变形（上下桨叶初始位置相同）
    DO I=1,THIS%NSTP,(THIS%NSTP-1)/2/THIS%AEROF%nrbd
        WRITE(89,*) I
        DO J=THIS%BLADE%NELE,1,-1
            WRITE(89, 995) THIS%BLADE%ELEMEN(THIS%BLADE%NELE-J+1)%NOD(1)%RLUD,VM(J+1,2*I-1)/THIS%BLADE%R*100,WM(J+1,2*I-1)/THIS%BLADE%R*100,FM(2*J+1,I)/PI*180
        END DO
        WRITE(89, 995) THIS%BLADE%ELEMEN(THIS%BLADE%NELE)%NOD(2)%RLUD,VM(1,2*I-1)/THIS%BLADE%R*100,WM(1,2*I-1)/THIS%BLADE%R*100,FM(1,I)/PI*180
    END DO    
   ...
   OPEN(444,FILE='cl3_rotor.dat')
    ...
    DO I=1,THIS%AEROF%NSTP!按站位循环
        DO J=1,NPTS!按展向气动点循环
...
REAL(RDT) :: PPSI,tmp
    ...
    write(122, 995) THIS%AEROF%mu,THIS%HUBFC1(1:3)*THIS%AEROF%trf/THIS%NODIMP1,&!THIS%trf=THIS%RHA*PI*OMG**2*R**4,THIS%NODIMP1=INPU%GETMASS()*THIS%omg**2*INPU%MAT%R**2
    THIS%HUBMT1(1:3)*THIS%AEROF%mrf/THIS%NODIMP2,&!THIS%MRF=THIS%trf*R,THIS%NODIMP2=INPU%GETMASS()*THIS%omg**2*INPU%MAT%R**3
    THIS%HUBFC0(1:3),THIS%HUBMT0(1:3)
    close(122)
...
SUBROUTINE GET_DEFORM(THIS,N,NumofRotors)
    ...
    !IF(N .EQ. 1) THEN
    !    open(33, file='deform1.dat')
    !ELSE IF(N .EQ. 2) THEN
    !    open(33, file='deform2.dat')
    !END IF
	...
	DO k=1,THIS%nstp!按站位循环
		THIS%ppsi=THIS%AEROF%PSIV(k)
		THIS%BLADE%UQS=THIS%UQSA(:, k)
		THIS%BLADE%UQS1T=THIS%UQS1TA(:, k)
		THIS%BLADE%UQS2T=THIS%UQS2TA(:, k)
		CALL THIS%BLADE%UPDATE()
        ...
        do i=1,101
            ...
            WRITE(33,995) (RXUD-RLUD(1))/(RLUD(NNOD)-RLUD(1))
            WRITE(33,995) REA(1:3)/THIS%BLADE%R
            WRITE(33,995) (TED(1:3,j),j=1,3)!输出肖宇论文中Tdc
            WRITE(33,995) (TRE(1:3,j),j=1,3)!输出肖宇论文中Teb乘Tbr结果
            WRITE(33,995) 
        end do
    END DO
...
INTEGER :: I,J,K,ERR
    ...
    CALL THIS%AEROF%UNIFLOW()!计算单个旋翼的平均诱导速度THIS%AEROF%LAMI
!!    write(*,*) lami,lamd,mu,ctc,ALP
    IMSEL:SELECT CASE(THIS%AEROF%MKIM)
        CASE(0)!前飞均匀入流
            ...
            DO I=1,NPTS
                DO J=1,THIS%NSTP
                    ...
                    THIS%AEROF%BVQA(3,I,J,1)=-THIS%AEROF%LAMIL*OMG*R
                END DO
            END DO
        ...
        CASE(1)!前飞线性入流
            ...
            DO I=1,NPTS!按气动插值点循环
                DO J=1,THIS%NSTP!按一圈站位数循环
                    PSI=HRAD*(J-1)
                    RB=RLUDAUD(I)
                    CALL THIS%AEROF%LINFLOW(rb,PSI,1)!上下旋翼判断:1上；-1下
                    THIS%AEROF%BVQA(3,I,J,1)=-THIS%AEROF%LAMIL*OMG*R
                END DO
            END DO
        ...
        CASE(5)!共轴入流
            ...
            !THETA=(/13.54,5.59,-9.12,16.58,4.05,-11.32/)!共轴
            !THETA=(/9.15,2.75,-5.34,10.59,5.45,9.36/)!共轴
            
            !CALL get_vi_forward(THETA,THIS%AEROF%BVQA(3,:,1:THIS%NSPPR,1),THIS%AEROF%BVQA(3,:,1:THIS%NSPPR,2),THIS%AEROF%LAMI*OMG*R)!共轴
            CALL COAXIALINFLOWMAIN(THIS%AEROF%BVQA(3,:,1:THIS%NSPPR,1),THIS%AEROF%BVQA(3,:,1:THIS%NSPPR,2),THIS%AEROF%LAMI*OMG*R)!共轴
            THIS%AEROF%BVQA(3,:,THIS%NSTP,1)=THIS%AEROF%BVQA(3,:,1,1)!共轴
            THIS%AEROF%BVQA(3,:,THIS%NSTP,2)=THIS%AEROF%BVQA(3,:,1,2)!共轴
            THIS%AEROF%BVQA(3,:,:,:)=-THIS%AEROF%BVQA(3,:,:,:)!共轴
