...
real(rdt),INTENT(OUT):: bvqa(3,NPTS,nstp)
...
!            end if
    ...
    do i=1,this%na
        do j=1,THIS%ns+1
            call this%sub_vi(i,THIS%bpq(:,j,i),BVQA(:,j,1+(i-1)*THIS%nx))
        end do
    end do
     BVQA(:,:,nstp)=BVQA(:,:,1)
    do i=1,this%na
        do j=1,THIS%nx
            bvqa(:,:,j+(i-1)*THIS%nx)=bvqa(:,:,1+(i-1)*THIS%nx)+&
                                              (bvqa(:,:,1+i*THIS%nx)-bvqa(:,:,1+(i-1)*THIS%nx))/THIS%nx*(j-1.0)
        end do
    end do
