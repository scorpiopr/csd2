...
INTEGER :: I
    ...
    THIS%NSTP=NSTP
    ALLOCATE(THIS%BVQA(3,NPTS,nstp),THIS%PSIV(NSTP))
...
REAL(RDT) :: C
    IF(.NOT.ALLOCATED(RLUDAUD)) THEN
        ...
        DO I=1,NNOD
            RLUD(I)=INPU%MAT%RL(I)!桨叶展向相对坐标
        END DO

        DO I=1,NPTS
            RLUDAUD(I)=(RLUD(NNOD)-RLUD(1))/(NPTS-1)*(I-1)+RLUD(1)!桨叶气动插值点展向坐标
        END DO
...
REAL(RDT),INTENT(IN) :: TH75
    GETCT=THIS%ALCSRF*THIS%SGSLID/2.0*(TH75/3.0-THIS%LAMI/2.0)
...
CLASS(AERO) :: THIS
    ...
    CALL THIS%INF%VEQ3(THIS%ALP,THIS%CTC/2,THIS%MU,THIS%LAMI,THIS%LAMD)!单个旋翼拉力系数为总拉力系数的一半
...
END SUBROUTINE
!->+++++++++++++++++++++++++++++++++++++++++++++
SUBROUTINE LINFLOW(THIS,rb,PSI)
IMPLICIT NONE
CLASS(AERO) :: THIS
REAL(RDT),INTENT(IN) :: RB,PSI

    CALL THIS%INF%linearinflow(THIS%lamil, THIS%lami, THIS%lamd, THIS%mu, rb, psi)
...
real(rdt) :: SSI,SSJ,ZERO=0.0
    ...
    ELSE IF(SOL.EQ.2) THEN
        ...
        DO I=1,3
            CALL INTP2D(INDVL(I),SSI,SSJ,THIS%bvqa(I,NJ,NI),THIS%bvqa(I,NJ,NI+1),THIS%bvqa(I,NJ+1,NI),THIS%BVQA(I,NJ+1,NI+1))
        END DO
...
100 CONTINUE        
    ...
    THIS%FLCSTCOE(:,LBI)=(/FLCSTN(1:3)/(THIS%RHA*THIS%BSC*UDR2),       (/FLCSTN(1:3)/)/(THIS%RHA*THIS%BSC*UDR2)*LBMA**2/)
